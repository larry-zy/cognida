...
# 把每个 Agent 的汇总指标以 JSON 行落盘
METRICS_FILE=/tmp/agent-eval.jsonl .claude/skills/link-dev/agent-eval/scripts/test-agents-e2e.sh
```
...
```json
{"agent":"RAG-Agent","endpoint":"/api/v1/agent/knowledge/stream","turns":10,"total_ms":42310,"avg_ms":4231,"max_ms":8120,"min_ms":1204,"total_tools":14,"empty_answers":0,"per_turn_ms":[...],"passed":true}
```
...
- **临时产物**：SSE 流、`server.log`、`*.answer.txt`、`build.log` 落在 `mktemp` 目录，默认清理，`--keep-log` 保留。
...
| `KEEP_LOG` / `--keep-log` | `0` | 保留临时文件 |
| `--no-boot` | 关 | 复用已运行服务，不自启、不扫服务端日志 |
| `LINK_GO_DIR` | 自动推导 | 显式指定 link-go 目录 |
| `METRICS_FILE` | 空 | 指定则把每 Agent 汇总指标以 JSON 行落盘 |
