...
#      （详见 services/cognida-go/docs/deepseek-thinking-reasoning-content-fix.md）
...
# ---------- 路径：定位 cognida-go ----------
# 本脚本位于 .claude/skills/cognida-dev/agent-eval/scripts/，向上 5 级即仓库根。
# 优先用 COGNIDA_GO_DIR 覆盖，便于在非常规布局 / CI 下显式指定。
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# .claude/skills/cognida-dev/agent-eval/scripts/ 向上 5 级即仓库根
REPO_ROOT="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
if [[ -n "${COGNIDA_GO_DIR:-}" ]]; then
    GO_DIR="$COGNIDA_GO_DIR"
else
    GO_DIR="$REPO_ROOT/services/cognida-go"
fi
...
# 产物根目录：默认落到项目内 var/test-output/agent-eval/（已 gitignore），不再用 /tmp。
# 日志/指标/各轮 SSE 与回答默认全部持久保留在项目里，便于回溯与历次对比。
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/var/test-output/agent-eval}"
...
产物：日志 / 指标 / 各轮 SSE 与回答默认持久保留在项目内
  var/test-output/agent-eval/<时间戳>-<agents>-<pid>/（已 gitignore），可用 OUTPUT_ROOT 覆盖根目录。

环境变量:
  SERVER_URL        服务地址（默认 http://localhost:8080）
  REQUEST_TIMEOUT   单轮 SSE 最长等待秒（默认 120）
  KEEP_LOG          =1 连 server 二进制也保留（等价 --keep-log）
  COGNIDA_GO_DIR       显式指定 cognida-go 目录（默认从脚本位置推导）
  OUTPUT_ROOT       产物根目录（默认 <repo>/var/test-output/agent-eval）
...
cleanup() {
    ...
    if [[ -n "$WORK_DIR" && -d "$WORK_DIR" ]]; then
        # 产物默认持久保留在项目内 var/test-output/。仅清掉体积大的编译产物，
...
check_deps() {
    ...
    if [[ ! -f "$GO_DIR/go.mod" ]]; then
        error "无法定位 cognida-go 目录（${GO_DIR}）。可用 COGNIDA_GO_DIR 显式指定，或用 --no-boot 复用已运行服务。"
        exit 1
    fi
}
...
boot_server() {
    ...
    if curl -fsS "$SERVER_URL/health" >/dev/null 2>&1; then
        ...
        error "请先释放端口（见 cognida-dev/kill-port 技能），或改用 --no-boot 复用既有服务。"
        exit 1
    fi
...
main() {
    ...
    # 产物目录：项目内 var/test-output/agent-eval/<时间戳>-<agents>-<pid>/，持久保留
