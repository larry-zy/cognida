...
SERVER_URL=http://localhost:8080 .claude/skills/cognida-dev/agent-eval/scripts/test-agents-e2e.sh --no-boot

# 连编译出的 server 二进制也一并保留（日志/指标默认已持久保留）
.claude/skills/cognida-dev/agent-eval/scripts/test-agents-e2e.sh --keep-log

# 指标默认落到本次产物目录 metrics.jsonl；可显式指定路径覆盖
METRICS_FILE=/path/to/agent-eval.jsonl .claude/skills/cognida-dev/agent-eval/scripts/test-agents-e2e.sh

# 图谱模式：只跑 RAG，用关系型问法 + 指定已抽取图谱的库，断言 graph_query 确被触发
AGENTS=rag GRAPH_ENABLED=1 KB_IDS=<已抽取图谱的知识库id> \
  .claude/skills/cognida-dev/agent-eval/scripts/test-agents-e2e.sh --no-boot
```

> 产物落盘：日志 / 指标 / 各轮 SSE 与回答默认持久保留在项目内
> `var/test-output/agent-eval/<时间戳>-<agents>-<pid>/`（已 gitignore），不再用 `/tmp`，
> 便于历次结果回溯对比；根目录可用 `OUTPUT_ROOT` 覆盖。

> 兼容入口：`services/cognida-go/scripts/test-agents-e2e.sh` 仍可用，它只是转发到本技能脚本。
...
- **服务启动**：默认 `go build ./cmd/server` 到临时目录并以 `DEV_MODE=true`、`LOG_LEVEL=debug` 启动，自动 `source services/cognida-go/.env`；跑完经 EXIT/INT/TERM trap `kill` 服务（遵循 CLAUDE.md「任务完成必须终止服务进程」）。
- **就绪探测**：轮询 `$SERVER_URL/health`，进程早退即打印启动日志并失败退出。
- **依赖**：`curl`、`jq`（缺失即报错退出）；毫秒计时用系统自带 `perl`。
- **cognida-go 定位**：默认从脚本位置向上推导仓库根，`COGNIDA_GO_DIR` 可覆盖。
- **产物落盘**：SSE 流、`server.log`、`*.answer.txt`、`build.log`、`metrics.jsonl` 落在项目内 `var/test-output/agent-eval/<时间戳>-<agents>-<pid>/`（已 gitignore），默认持久保留；仅编译出的 server 二进制在退出时清掉，`--keep-log` 连二进制也留。根目录可用 `OUTPUT_ROOT` 覆盖。
...
| `COGNIDA_GO_DIR` | 自动推导 | 显式指定 cognida-go 目录 |
| `OUTPUT_ROOT` | `<repo>/var/test-output/agent-eval` | 产物根目录 |
...
- **kill-port**：若上次运行残留占用 8080，先用 `cognida-dev/kill-port` 清理再跑。
- **testing**：本技能聚焦「Agent 端到端多轮对话回归」；单元/集成/浏览器 E2E 走 `cognida-dev/testing`。
- **mock-login**：如需带认证访问，用 `cognida-dev/mock-login` 取测试用户 token。
