...
# 检查包名 - 不应有下划线
if grep -r '^package.*_' "$PROJECT_ROOT/services/cognida-go/internal/" 2>/dev/null | grep -v '_test.go' | grep -v '.git'; then
    echo "❌ 包名不应包含下划线"
    FAILED=1
fi
...
if grep -rE 'type I[A-Z][a-z]' "$PROJECT_ROOT/services/cognida-go/internal/" 2>/dev/null | grep -v '_test.go' | grep -v '.git'; then
...
if grep -rnE 'func[^{]*,[[:space:]]*[a-zA-Z_]+[[:space:]]+context\.Context' "$PROJECT_ROOT/services/cognida-go/internal/" 2>/dev/null | grep -v '_test.go' | grep -v '.git'; then
