...
if [ ! -d "$PROJECT_ROOT/apps/cognida-web" ] || [ ! -d "$PROJECT_ROOT/services/cognida-go" ]; then
    echo "⚠️  cognida-web 或 cognida-go 目录不存在，跳过"
    exit 0
fi

# 提取 Go 定义的路径 (简化检查)
if grep -rh 'POST\|GET\|PUT\|DELETE' "$PROJECT_ROOT/services/cognida-go/internal/handler/" 2>/dev/null | \
    grep -o '".*"' | sed 's/"//g' | sed 's/\\//g' | sort -u > /tmp/go_paths.txt; then
    echo "📋 发现 $(wc -l < /tmp/go_paths.txt) 个 Go API 路径"
fi

# 提取前端调用的路径
if grep -rh 'fetch.*api\|axios.*api' "$PROJECT_ROOT/apps/cognida-web/src/" 2>/dev/null | \
