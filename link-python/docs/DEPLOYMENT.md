...
```bash
docker build -t link-python:latest .
```
...
```bash
# 查看日志
docker logs link-python

# 检查配置
docker run --rm --env-file .env link-python:latest python -c "from config import get_settings; print(get_settings().model_dump())"
```
