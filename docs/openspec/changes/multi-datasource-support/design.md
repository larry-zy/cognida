...
cognida-go 目前以单一全局 DSN 初始化一个 `gorm.DB` 单例（`internal/repository/mysql/mysql.go`），所有查询都落在自身业务库上。text2sql 工具链已预留 `database_id` 参数（`internal/service/agent/tools/get_schema.go` 的 `resolveDatabase()`），但恒回落当前库。cognida-python 只从文件读数据，前端无数据源相关 UI。
...
- cognida-python 不直连外部数据库（Phase 3 由 Go 抽样后推数据/落 Parquet）。
...
1. Phase 1 合入后跑 `cd cognida-go && set -a && source .env && set +a && go run ./cmd/migrate-db` 建 `data_sources` 表；配置 `DATASOURCE_SECRET_KEY`。
