...
- 洞察归因（垂类差异化）：`data_analysis` 分化为趋势/对比/**归因-根因**/报告解读，归因经 Go 编排 + cognida-python 算法（大小模型协同，driver ranking + variance decomposition）。
...
- **cognida-go**：`internal/service/agent/` 新增 `presets/data_agent/`、`resultstore/`、`context/`；改造 `framework/eino_agent.go`（观察压缩 + 工具门）、`genui/`（Catalog 扩展 + render_ui）、`tools/`（新增 mutate/etl/export/render_ui，改造 sql_execute/get_schema）、`skills/`（硬工具门）、`initializer/init.go` 与 `cmd/server/main.go`（注册子代理）。
...
- **cognida-web**：`components/agent/A2UINode.vue`（交互回调）、`A2UIRenderer.vue`、`views/ai/AICenterView.vue`（多 UI surface + 确认卡片）。
- **cognida-python**：计算侧算法不变，`tools/analytics/` 按 result_id 契约对接归因 API。
