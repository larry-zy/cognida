...
- [x] 2a.1 新增 `agent_semantic_*` GORM model（逻辑表/维度/度量/指标/关系）+ 语义模型 repository；`cd cognida-go && set -a && source .env && set +a && go run ./cmd/migrate-db` 同步
...
- [x] 4.9 cognida-web：`A2UINode.vue` / `A2UIRenderer.vue` 交互回调，`AICenterView.vue` 多 UI surface；优先自研 Ui* 组件
...
- [x] 4a.2 归因/根因：Go 侧编排 + cognida-python 归因算法（variance decomposition + driver ranking + 隐藏因子），大小模型协同
- [x] 4a.3 归因结果回传 result_id 信封 + 文字洞察，附样本与口径/置信标注，支持下钻校验
- [x] 4a.4 cognida-python `tools/analytics/` 按 result_id 契约对接归因 API（计算侧算法不变）
- [x] 4a.5 单元测试：意图→能力路由、归因回传契约、口径标注、下钻引用
- [x] 4a.6 集成测试：真实数据归因端到端（driver ranking 稳定性）

## Phase 4 — 操作工具 + 危险分级 + 审计
- [x] 5.1 新增 `agent_operation_audit` GORM model；`cd cognida-go && set -a && source .env && set +a && go run ./cmd/migrate-db` 同步
...
- [x] 9.2 cognida-python `tools/analytics/` 按 result_id 契约对接（计算侧算法不变）
