...
`data_analysis` SHALL 分化为命名分析能力：趋势、对比、归因/根因、报告解读。归因/根因 SHALL 支持 variance decomposition 与 driver ranking，由 Go 侧编排 + cognida-python 侧算法（归因 API）协同（大小模型协同）。归因结果 SHALL 回传 `result_id` 信封 + 文字洞察，并附样本与口径标注。
