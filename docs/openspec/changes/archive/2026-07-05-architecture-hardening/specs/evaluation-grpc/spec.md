...
**Migration**: 调用方改为通过 Go worker 触发评测（任务状态与进度由 Go worker 管理），Python 侧仅提供无状态 `compute_*` 指标计算；移除前需 `grep ExecuteEvaluation cognida-go` 确认无 Go 侧调用，并同步删除 `proto` 中该 RPC、`cognida-python` 的 servicer 实现与 `runner.py` 状态机。
