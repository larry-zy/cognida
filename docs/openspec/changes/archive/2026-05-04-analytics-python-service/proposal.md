...
Cognida 项目当前缺乏数据分析能力，无法为 Agent 提供数据统计、趋势分析和洞察发现功能。实现这些能力是构建"AI 数据专家"的基础，也是支持智能问数、洞察报告生成等上层功能的关键。

## What Changes

在 `cognida-python` 服务中新增数据分析能力：
...
**新增代码**:
- `cognida-python/services/analytics/` - 数据分析核心模块
- `cognida-python/grpc_service/analytics_servicer.py` - gRPC 服务实现
- `cognida-go/api/proto/analytics.proto` - Protobuf 定义
