...
- **cognida-go**：`cmd/wire`（组合根注册 executor）、`internal/service/evaluation/worker.go`（type 路由）、`executor/agent.go`、`internal/handler/evaluation_handler.go`（校验 `agent_id`）、`internal/model/evaluation/entity.go`（数据集样本 Agent 字段）、`repository/mysql`（迁移新字段，走 `cmd/migrate-db`）。
- **cognida-web**：`views/evaluation/components/CreateEvaluationDialog.vue`、`composables/useEvaluationList.ts`、`api/agent`（新增 ListAgents 封装）、`router`（重命名）、`evaluation-config.ts`。
- **cognida-python**：新增 HF→评测数据集转换脚本（复用 `services/dataset/loader.py`）、Agent 指标输入所需字段。
