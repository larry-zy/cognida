...
系统 SHALL 以顶层 `proto/` 目录作为跨服务 proto 契约的唯一 source-of-truth，收纳 `analytics`/`docreader`/`evaluation`/`judge`/`quality` 等 `.proto`。系统 MUST NOT 在 `link-go/api/proto` 与 `link-python/proto` 各维护一份手抄 `.proto`。

#### Scenario: 单一源目录存在

- **WHEN** 检查仓库顶层
- **THEN** 存在 `proto/` 目录，包含全部跨服务 `.proto` 定义
- **AND** `link-go/api/proto` 与 `link-python/proto` 下 MUST NOT 再保留手抄的 `.proto` 源文件
...
- **THEN** 系统 SHALL 从 `proto/` 生成 Go stub 到 link-go 生成目录
- **AND** SHALL 同时生成 Python stub 到 link-python 生成目录
