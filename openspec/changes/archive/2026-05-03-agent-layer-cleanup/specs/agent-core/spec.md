...
### Requirement: ChatChunk in domain layer
**Reason**: `ChatChunk` is a runtime DTO for streaming responses, belongs in Application layer
**Migration**: Use `ChatChunkDTO` from `application/usecases/agent/dto.go`

### Requirement: ToolCallRecord in domain layer
**Reason**: `ToolCallRecord` contains execution details, is a DTO, belongs in Application layer
**Migration**: Use `ToolCallRecordDTO` from `application/usecases/agent/dto.go`

### Requirement: AgentOrchestrator in domain layer
**Reason**: Orchestration is an application layer concern (use case coordination), not a domain concept
**Migration**: Define orchestrator interfaces in Application layer use cases
