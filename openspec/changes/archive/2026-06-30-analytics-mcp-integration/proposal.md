...
- **Python (`link-python`)**：新增 `tools/analytics/`（MCP 工具包装）与 `tools/bootstrap.py`（注册引导）；`mcp/server.py` 初始化处调用注册；MCP 以 http 模式常驻以供 Go 调用。
- **Go (`link-go`)**：新增 `internal/service/agent/tools/data_analysis.go`；在 `internal/service/agent/tools/init.go` 注册；`framework/hooks/conclusion.go` 将 `data_analysis` 纳入数据工具；复用 `infrastructure/config` 的 MCP endpoint 配置与 `infrastructure/mcp.MCPClient`。
