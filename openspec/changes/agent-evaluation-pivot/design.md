...
- **数据集导入弱**：只能手工上传 JSONL；`link-python/services/dataset/loader.py` 有 `load_hf_dataset` 但未接入评测数据集格式与灌库流程；无评测数据集 seed 脚本。
...
1. 加样本字段 → `cd link-go && go run ./cmd/migrate-db` 同步表结构。
