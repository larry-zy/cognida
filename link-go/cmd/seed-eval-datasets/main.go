...
//        cd link-go && set -a && source .env && set +a && go run ./cmd/seed-eval-datasets --strict
//
...
// 并提示。重建电商库后，务必重跑：
//   cd link-python && .venv/bin/python scripts/convert_eval_datasets.py --only scenario_ecommerce_agent
