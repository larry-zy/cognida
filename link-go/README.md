...
在 LLM/Text2SQL Agent 与数仓之间引入一层**受治理的指标语义层**：把「LLM 直接裸写 SQL」升级为「引擎依据中心化的指标口径生成 SQL」。全部实现位于 `link-go`（Python 端无语义层）。
...
```bash
cd link-go && set -a && source .env && set +a && go run ./cmd/migrate-db
```
