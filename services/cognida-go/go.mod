...
// golang-migrate v4.19.1 传递性要求 otelgrpc v0.61.0，但该版本移除了
// UnaryClientInterceptor/UnaryServerInterceptor（改用 stats handler），
// 而本仓库 tracing 拦截器与 etcd v3.5.23 server 包仍依赖旧 API。
// golang-migrate 在本项目编译路径（core + mysql + iofs 源）并不引入 otelgrpc，
// 故固定回 v0.59.0；待整体迁移到 stats handler 后移除本 replace。
