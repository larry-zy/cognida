...
// headerRecordingClient 记录最近一次 POST 的头，用于验证 request_id 透传。
// 实现 HeaderPoster（带头 POST）与 HTTPClient（无头 POST）两条路径。
...
func (m *headerRecordingClient) Post(url string, body []byte) ([]byte, error) {
	m.postCalled = true
	m.lastHeaders = nil
	return m.response, nil
}

func (m *headerRecordingClient) PostWithHeaders(url string, body []byte, headers map[string]string) ([]byte, error) {
	m.lastHeaders = headers
	return m.response, nil
}

// plainClient 仅实现 HTTPClient（不带头），用于验证回退路径。
type plainClient struct {
	postCalled bool
	response   []byte
}

func (m *plainClient) Post(url string, body []byte) ([]byte, error) {
	m.postCalled = true
	return m.response, nil
}

func mcpToolSuccessResponse() []byte {
	resp := map[string]interface{}{
		"jsonrpc": "2.0",
		"id":      "test",
		"result": []interface{}{
			map[string]interface{}{"type": "text", "text": `{"success": true, "data": {}}`},
		},
	}
...
func TestMCPClient_Invoke_ForwardsRequestID(t *testing.T) {
	mock := &headerRecordingClient{response: mcpToolSuccessResponse()}
	...
	ctx := agentctx.WithRequestID(context.Background(), "rid-abc-123")
	_, err = client.Invoke(ctx, "analytics", map[string]interface{}{"data": []int{1}})
	require.NoError(t, err)

	require.NotNil(t, mock.lastHeaders, "应通过 PostWithHeaders 透传请求头")
	assert.Equal(t, "rid-abc-123", mock.lastHeaders[HTTPRequestIDHeader])
}

func TestMCPClient_Invoke_NoRequestID_UsesPlainPost(t *testing.T) {
	// 有 HeaderPoster 能力但 ctx 无 request_id：不应附带头，走无头 Post。
	mock := &headerRecordingClient{response: mcpToolSuccessResponse()}
	client, err := NewMCPClient(&Config{
		Endpoint:   "http://localhost:8080/mcp",
		HTTPClient: mock,
	...
	_, err = client.Invoke(context.Background(), "analytics", map[string]interface{}{})
	require.NoError(t, err)

	assert.True(t, mock.postCalled, "无 request_id 时应回退到无头 Post")
	assert.Nil(t, mock.lastHeaders)
}

func TestMCPClient_Invoke_PlainClient_Fallback(t *testing.T) {
	// 传输层未实现 HeaderPoster：即便有 request_id 也安全回退到 Post，不报错。
	mock := &plainClient{response: mcpToolSuccessResponse()}
	client, err := NewMCPClient(&Config{
		Endpoint:   "http://localhost:8080/mcp",
		HTTPClient: mock,
	})
	...
	ctx := agentctx.WithRequestID(context.Background(), "rid-xyz")
	result, err := client.Invoke(ctx, "analytics", map[string]interface{}{})
	require.NoError(t, err)
	assert.True(t, mock.postCalled)
