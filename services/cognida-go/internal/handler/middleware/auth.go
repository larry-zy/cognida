...
func (m *AuthMiddleware) Apply() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if authHeader == "" && c.Request.Method == http.MethodGet {
			// 仅 GET 允许从 Query 取 token〔INF-3〕：SSE/EventSource 无法自定义请求头，
			// 只能走 ?token=；对可变更操作（POST/PUT/DELETE）不再接受 URL query token，
			// 缩小其在日志/Referer 中泄漏的暴露面。
			authHeader = c.Query("token")
		}

		// 开发模式绕过认证（仅用于开发环境）——附加"本机来源"硬约束〔INF-3〕：
		// 生产环境即使误设 DEV_MODE=true，只要请求非来自 loopback 即拒绝绕过，
		// 用 RemoteIP()（真实 TCP 对端，不可被 X-Forwarded-For 伪造）判定。
		if authHeader == "" && os.Getenv("DEV_MODE") == "true" && !isLoopbackAddr(c.RemoteIP()) {
			log.Printf("[Auth][SECURITY] DEV_MODE 绕过被拒绝：请求非本机来源 remote=%s %s %s",
				c.RemoteIP(), c.Request.Method, c.Request.URL.Path)
			// 不绕过：继续走正常认证流程（authHeader 为空 → 下方返回 401）。
		} else if authHeader == "" && os.Getenv("DEV_MODE") == "true" {
...
// isLoopbackAddr 判断给定 IP 字符串是否为本机回环地址（127.0.0.0/8 或 ::1）。
