...
import (
	...
	"cognida/internal/service/agent/agentstate"
...
type AgentHandler struct {
	...
	// genuiModel 生成式 UI（Level 2）所用 LLM，由组合根经 SetGenUIModel 注入，
	// 替代 genui 包级单例（消除隐藏全局态〔GO-2〕）。为 nil 时 GenUI 退化为纯模板（Level 1）。
	genuiModel genui.LLM
}
...
// SetGenUIModel 注入生成式 UI（Level 2）所用 LLM，由组合根在构建 toolModel 后调用一次。
// 传 nil 表示禁用 Level 2（GenUI 退化为确定性模板）。
func (h *AgentHandler) SetGenUIModel(m genui.LLM) {
	h.genuiModel = m
}
