// Package prompt 是全项目提示词（Prompt）的集中配置中心。
//
// 设计要点：
//   - 每个业务一个 YAML 文件（configs/<business>.yaml），文件内为「key: 多行文本」的
...
//
// 与知识图谱抽取用的 config.LoadPromptTemplate（运行时按 cwd 读文件）相互独立：
// 那套服务于图谱抽取/查询改写的模板，仍保留在 config/prompt_templates/ 原地。
package prompt
...
// load 解析 configs/ 下所有 YAML，返回 business -> key -> content 的两级映射。
func load() (map[string]map[string]string, error) {
	entries, err := configFS.ReadDir("configs")
	if err != nil {
		return nil, fmt.Errorf("读取内嵌 configs 目录: %w", err)
	}
	...
	for _, e := range entries {
		...
		data, err := configFS.ReadFile("configs/" + e.Name())
		if err != nil {
...
func MustGet(business, key string) string {
	...
	if !ok {
		panic(fmt.Sprintf("prompt: 缺少提示词 %q/%q（请检查 internal/prompt/configs/%s.yaml）", business, key, business))
	}
	return v
}
