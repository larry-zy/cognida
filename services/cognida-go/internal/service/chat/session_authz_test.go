...
func ctxWithIdentity(userID, tenantID int64) context.Context {
	ctx := agentctx.WithUserID(context.Background(), userID)
...
func TestAuthz_MissingIdentity_Denied(t *testing.T) {
	...
	cases := []struct {
		...
		{"仅有 user_id 缺 tenant_id", agentctx.WithUserID(context.Background(), int64(100))},
		{"仅有 tenant_id 缺 user_id", agentctx.WithTenantID(context.Background(), int64(1))},
