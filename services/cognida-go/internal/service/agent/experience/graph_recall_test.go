...
func TestRecall_SameNameExperiencesNotOverwritten(t *testing.T) {
	// 两条不同经验共用同一 Name（沉淀去重前的历史残留）：应各自入桶、均被召回，
	// 而非后者覆盖前者只剩一条。命中该 name 的边为桶内每条经验同时计分。
	data := &knowledge.GraphData{
		Node: []*knowledge.GraphNode{
			{ID: "exp_5", Name: "经验：重名经验", EntityType: "experience",
				Properties: map[string]string{"problem": "p5", "solution": "s5"}},
			{ID: "exp_6", Name: "经验：重名经验", EntityType: "experience",
				Properties: map[string]string{"problem": "p6", "solution": "s6"}},
			{ID: "topic_电商分析", Name: "电商分析", EntityType: "topic"},
		},
		Relation: []*knowledge.GraphRelation{
			{ID: "sr1", Source: "经验：重名经验", Target: "电商分析", Type: string(knowledge.RelationTypeRelatedTo)},
		},
	}
	r := NewGraphRecaller(&fakeGraphRepo{data: data})
	got := r.Recall(context.Background(), 1, "电商分析", 5)
	if len(got) != 2 {
		t.Fatalf("同名两条经验应均被召回, got %d: %+v", len(got), got)
	}
	if got[0].Score != 4 || got[1].Score != 4 {
		t.Errorf("同名桶内每条经验应各得 4 分, got %d / %d", got[0].Score, got[1].Score)
	}
}

func TestRecall_DuplicateEdgesScoredOnce(t *testing.T) {
	// GraphSink 对同一话题可能发多条 RELATED_TO：去重后同一关联名对该经验只计一次，
	// 不因重复边把分数翻倍。
	data := &knowledge.GraphData{
		Node: []*knowledge.GraphNode{
			{ID: "exp_7", Name: "经验：去重经验", EntityType: "experience",
				Properties: map[string]string{"problem": "p7", "solution": "s7"}},
			{ID: "topic_电商分析", Name: "电商分析", EntityType: "topic"},
		},
		Relation: []*knowledge.GraphRelation{
			{ID: "dr1", Source: "经验：去重经验", Target: "电商分析", Type: string(knowledge.RelationTypeRelatedTo)},
			{ID: "dr2", Source: "经验：去重经验", Target: "电商分析", Type: string(knowledge.RelationTypeRelatedTo)},
		},
	}
	r := NewGraphRecaller(&fakeGraphRepo{data: data})
	got := r.Recall(context.Background(), 1, "电商分析", 5)
	if len(got) != 1 {
		t.Fatalf("应召回 1 条, got %d", len(got))
	}
	if got[0].Score != 4 {
		t.Errorf("重复边应只计一次(得 4)，而非累加翻倍, got %d", got[0].Score)
	}
}

func TestRecall_GraphCachedWithinTTL(t *testing.T) {
	// 同租户 TTL 内多次召回应复用缓存，只拉一次全图，避免每 turn 一次 DB 往返。
	repo := &fakeGraphRepo{data: expSubgraph()}
	r := NewGraphRecaller(repo)
	for i := 0; i < 3; i++ {
		r.Recall(context.Background(), 1, "电商分析", 3)
	}
	if repo.calls != 1 {
		t.Errorf("TTL 内同租户应只拉一次全图, got %d 次", repo.calls)
	}
	// 不同租户各自独立拉取。
	r.Recall(context.Background(), 2, "电商分析", 3)
	if repo.calls != 2 {
		t.Errorf("新租户应触发新的一次拉取, got %d 次", repo.calls)
	}
}
