...
// 意图 → 确定性绑定领域技能名的映射：趋势/归因/报告有专门技能，其余无绑定（空串）。
func TestSkillFor(t *testing.T) {
	cases := []struct {
		intent Intent
		want   string
	}{
		{IntentTrend, "trend-analysis"},
		{IntentAttribution, "attribution-analysis"},
		{IntentReport, "report-composition"},
		{IntentFetch, ""},
		{IntentGeneral, ""},
		{IntentAmbiguous, ""},
	}
	for _, c := range cases {
		if got := SkillFor(c.intent); got != c.want {
			t.Errorf("SkillFor(%v) = %q, want %q", c.intent, got, c.want)
		}
	}
}
