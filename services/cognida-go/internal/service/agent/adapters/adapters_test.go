...
import (
	...
	svcknowledge "cognida/internal/service/knowledge"
)
...
func TestRAGAdapter_SessionParamsReachRetrieveOptions(t *testing.T) {
	...
	if _, err := a.Query(ctx, &tools.RAGQueryRequest{Query: "hi", TopK: 5, MinScore: float64Ptr(0.9), EnableRerank: false}); err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if spy.last == nil {
...
// 无会话级参数时，工具即兴 MinScore 应原样透传底层（含显式 0=关闭分数过滤）。
// 覆盖指针化后「显式值原样传、不再被吞」的适配层分支。
func TestRAGAdapter_ToolMinScorePassthroughWhenNoSession(t *testing.T) {
	cases := []struct {
		name string
		in   *float64
		want float64
	}{
		{"explicit positive", float64Ptr(0.7), 0.7},
		{"explicit zero disables filter", float64Ptr(0), 0},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			a := newTestAdapter(spy, repo)
			ctx := agentctx.WithAllowedKBIDs(baseCtx(), []string{"kb1"})
			if _, err := a.Query(ctx, &tools.RAGQueryRequest{Query: "hi", TopK: 5, MinScore: tc.in}); err != nil {
				t.Fatalf("unexpected err: %v", err)
			}
			if spy.last == nil {
				t.Fatalf("retriever not called")
			}
			if float64(spy.last.SimilarityThreshold) != tc.want {
				t.Fatalf("expected opts.SimilarityThreshold=%v, got %v", tc.want, spy.last.SimilarityThreshold)
			}
		})
	}
}
