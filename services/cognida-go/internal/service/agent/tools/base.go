...
// extractUserID 从上下文提取用户ID（审计用，字符串形态）。
// 统一走 agentctx 类型化 key：入口以 int64 注入，未设置或非正值返回空串。
// 旧实现读字符串 key 且断言为 string，与全局 int64 存储不符，恒为空——本函数修正之。
func extractUserID(ctx context.Context) string {
	if uid, ok := agentctx.GetUserID(ctx); ok && uid > 0 {
		return strconv.FormatInt(uid, 10)
	}
	return ""
}

// extractSessionID 从上下文提取会话ID（审计用）。同样统一走 agentctx 类型化 key。
func extractSessionID(ctx context.Context) string {
	return agentctx.MustGetSessionID(ctx)
}
