...
func ensureLimit(sqlStr string, maxRows int) string {
	...
	// 解析失败或超上限一律把 count 压到 maxRows（仅改写这一处尾部 LIMIT）。
	// 关键：`LIMIT offset, count` 形态必须保留 offset，只封顶 count——否则翻页页码错位
	// （如 `LIMIT 100, 5000` 被封成 `LIMIT 1000` 会拿到第 1 页而非目标页）。
	rowStr := m[1]
	hasOffset := m[2] != ""
	if hasOffset {
		rowStr = m[2]
	...
	var capped string
	if hasOffset {
		capped = fmt.Sprintf("LIMIT %s, %d", m[1], maxRows) // 保留原 offset，仅封顶 count
	} else {
		capped = fmt.Sprintf("LIMIT %d", maxRows)
	}
	return tailLimitRe.ReplaceAllString(trimmed, capped)
}
