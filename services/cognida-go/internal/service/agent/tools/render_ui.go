...
- /table            → {columns, rows}（有界快照）；取单个单元格用 /table/rows/<行号>/<列名>，
                      如榜首名称 /table/rows/0/商品名称。注意 rows 挂在 /table/rows 下，写 /table/0/... 解析不到
- /series           → {labels, actual, forecast}（LineChart 时序 / BarChart 分类对比）
- /scatter          → {x, y, xLabel, yLabel}（ScatterChart 两数值列相关性，仅在有该路径时）
- /metrics/<名称>    → 单个指标值（来自 data_analysis；普通 SQL 取数没有此路径，要单元格值请走 /table/rows/<行号>/<列名>）
...
[
  ...
  {"id":"m1","type":"MetricCard","props":{"label":"榜首商品","value":{"path":"/table/rows/0/商品名称"}}},
