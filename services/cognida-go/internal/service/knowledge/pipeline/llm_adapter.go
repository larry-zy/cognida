...
func (a *LLMChatAdapter) Chat(ctx context.Context, messages []domainrag.LLMMessage, opts *domainrag.ChatOptions) (*domainrag.LLMResponse, error) {
	...
	// domainrag.ChatOptions 用 float64（无「未设置」态）：沿用原语义，仅 >0 才下传，
	// 0 视作未设置、回落 provider 默认——与改指针前行为一致，避免 RAG 生成路径回归。
	if opts.Temperature > 0 {
		req.Options.Temperature = llm.Float64Ptr(opts.Temperature)
	}
	if opts.TopP > 0 {
		req.Options.TopP = llm.Float64Ptr(opts.TopP)
	}
