...
func (r *EvaluationTaskRepository) List(ctx context.Context, filter *evaluation.TaskFilter) ([]*evaluation.EvaluationTask, int64, error) {
	query := r.getDB(ctx).Model(&EvaluationTaskModel{}).Where("deleted_at IS NULL")
...
// getDB 获取数据库连接（支持事务）：优先复用 ctx 中的事务句柄，否则回退到基础连接。
func (r *EvaluationTaskRepository) getDB(ctx context.Context) *gorm.DB {
