...
// txContextKey 事务在 context 中的键。使用私有类型避免与其它包的字符串键冲突，
// 并让 go vet 的 SA1029 检查通过（禁止用内建类型作为 context key）。
type txContextKey struct{}

// contextWithTx 将事务绑定到新的 context。
func contextWithTx(ctx context.Context, tx *gorm.DB) context.Context {
	return context.WithValue(ctx, txContextKey{}, tx)
}

// DBFromContext 返回应当用于本次操作的 *gorm.DB：
// 若 ctx 中携带了由 WithTransaction 注入的事务句柄，则复用该事务（保证原子性）；
// 否则回退到传入的 fallback 连接。所有仓储都应通过此函数（或 BaseRepository 的
// 访问器）获取 DB，才能真正参与上层开启的事务。
...
func (tm *gormTransactionManager) WithTransaction(ctx context.Context, fn func(ctx context.Context) error) error {
	return tm.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		// 将事务句柄注入 context，供仓储经 DBFromContext 取用
