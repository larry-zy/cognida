...
// Float64Ptr 返回 v 的指针，便于构造带显式采样值（含确定性的 0）的 ChatOptions。
func Float64Ptr(v float64) *float64 { return &v }

// ChatOptions 聊天选项。
//
// Temperature/TopP 为指针：nil=未设置（回落 provider 默认），非 nil=显式指定（含 0）。
// 用 float64 无法区分「未设置」与「显式 0」，会让确定性采样（temperature:0，评测/裁判需要）
// 被 `> 0` 守卫静默丢弃回落 provider 默认（常 0.7）；改指针后显式 0 得以如实下传。
