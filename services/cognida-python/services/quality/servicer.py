...
class QualityServicer(quality_pb2_grpc.QualityServiceServicer):
    ...
    def RegisterCustomEvaluator(
        ...
        """注册自定义评估器。"""
        try:
            # 安全（PY-5）：不再对请求方传入的 module_path 做任意 import；
            # 通过受控白名单加载，且校验目标是评估器基类子类。
            from services.common.plugin import load_plugin_class
            from services.quality.dimensions.base import DimensionEvaluator
            from services.quality.unstructured.base import UnstructuredEvaluator

            evaluator_class = load_plugin_class(
                request.module_path,
                request.class_name,
                allowed_prefixes=(
                    "services.quality.dimensions",
                    "services.quality.unstructured",
                ),
                expected_base=(DimensionEvaluator, UnstructuredEvaluator),
            )
    ...
    def RegisterCustomCleaner(
        ...
        """注册自定义清洗器。"""
        try:
            # 安全（PY-5）：受控白名单加载，禁止请求驱动的任意模块 import。
            # 清洗器基类有 Cleaner 与 TextCleaner 两支（历史沿用），均纳入校验。
            from services.common.plugin import load_plugin_class
            from services.quality.cleaning.base import Cleaner, TextCleaner

            cleaner_class = load_plugin_class(
                request.module_path,
                request.class_name,
                allowed_prefixes=("services.quality.cleaning",),
                expected_base=(Cleaner, TextCleaner),
            )
