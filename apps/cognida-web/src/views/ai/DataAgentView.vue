...
// AI/Markdown 渲染唯一入口（内含 DOMPurify 消毒，防 XSS）
...
// ===== 持久化解析 =====
// 思考步骤（历史时间线）解析由 useAgentChat 的 parseAgentSteps 提供。
...
const {
  ...
  deleteSession,
  formatSessionTime
} = useChatSessions<DataMessage>({
  messages,
  currentSessionId,
  inputMessage,
  textareaRef,
  filter: s => s.agent_type === 'text2sql',
  mapMessage: msg => ({
    ...msg,
  ...
  searchTitleFallback: '新分析',
  deleteConfirmMessage: '确定要删除这条分析记录吗？',
  loadFailMessage: '加载分析记录失败',
...
const {
  ...
  createStream: (content, signal) => agentApi.streamDataChat({
    query: content,
  ...
  // 快照 steps / ui_surfaces；成功与异常/中断分支共用，确保已产出内容不「消失」。
  ...
  onError: (err, ctx) => {
    if (err?.name === 'AbortError') {
      ...
      if (ctx.state.assistantContent || ctx.streamingContent || ctx.streamingSteps.length || streamingUISurfaces.value.length) {
        ctx.commit((ctx.state.assistantContent || ctx.streamingContent || '').trim() || '（生成已中断）')
      }
    } else {
      ...
      const partial = (ctx.state.assistantContent || ctx.streamingContent || '').trim()
      ctx.commit(
        (partial ? partial + '\n\n' : '') +
...
onUnmounted(() => {
  // SSE 中断由 useAgentChat 的 onUnmounted 处理，这里仅解绑全局点击关闭
