...
| promtail | — | 采集器：抓取宿主机 `link-go/logs` 与 `link-python/logs` 下的 `*.log` |
| grafana | 3300 | 查询/可视化，已预置 Loki 数据源（匿名 Admin，免登录）|

## 前置：让服务把日志落到被采集目录

- **Python**：`link-python/scripts/dev-all.sh` 已把四服务日志写到
  `link-python/logs/{grpc,http,eval,mcp}.log`，开箱即用。
- **Go**：stdout 需重定向到 `link-go/logs/`：
  ```bash
  cd link-go
...
- **低基数 label**：`job=link` / `stack=go|python` / `service=<文件名>`。
